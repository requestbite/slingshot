...
export function RequestPage() {
  ...
  const loadRequest = async (requestId) => {
    try {
      ...
      // Full page reload to root if request not found
    ...
    return (
      <div class="flex items-center justify-center h-full">
        <div class="text-center">
          ...
          <h3 class="text-lg font-medium text-gray-900 dark:text-neutral-dark-900 mb-2">Failed to load request</h3>
          <p class="text-gray-600 dark:text-neutral-dark-600 mb-4">{error}</p>
          <button
  ...
  if (!request && !isLoading) {
    // If no request is loaded and we're not loading, full page reload to root
    window.location.href = '/';
    return null;
  }
