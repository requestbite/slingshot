...
function OperationItem({ method, path, operation, isActive, onClick, level = 0 }) {
  ...
  return (
    <li class="request relative">
      <div
        class={`flex items-center py-1 px-1 rounded cursor-pointer hover:bg-gray-100 ${isActive ? 'bg-sky-50' : ''}`}
...
function TagFolder({ tag, searchTerm, activeId, onSelect, level = 0 }) {
  ...
  return (
    <li class="folder">
      ...
      {!isDefault && (
        <div
          class="flex items-center text-gray-700 py-1 hover:bg-gray-100 rounded px-1 cursor-pointer"
          style={{ marginLeft: `${marginLeft}px` }}
      ...
      <div class="p-3 border-b border-gray-200 flex-shrink-0">
        <div class="relative">
          ...
          <input
            ...
            class="w-full pl-7 pr-2 py-1.5 text-xs rounded-md border border-gray-300 focus:outline-none focus:border-sky-500 focus:ring-1 focus:ring-sky-500"
          />
          {searchTerm && (
            <button
